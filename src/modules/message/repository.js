...
export const LIMIT_TRANSACTION = 256
...
export const getTransactions = async (room, offset, listener = undefined) => {
  if (!room) {
    return []
  }
  let instance = null
  const ref = firebase.db
    .collection(`rooms/${room.id}/transactions`)
    .where('time', '>', offset)
...
export const sendTransactions = async (user, room, data) => {
  const time = firebaseApp.firestore.Timestamp.now()
  const newRef = firebase.db.collection(`rooms/${room.id}/transactions`).doc()
  return newRef.set({
    ...data,
    time: time.toMillis()
  }).then(response => {
