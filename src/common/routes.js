...
export default class Root extends Component {
  render () {
    ...
    const { user: { userInformation } } = store.getState()
    return (
      <div>
        <Route exact path='/' component={Page(userInformation ? HomePage : LoginPage)} />
        <Route path='/signup' component={Page(SignUpPage)} />
        <Route path='/login' component={Page(LoginPage)} />
        <Route path='/verify' component={Page(VerifyPage)} />
